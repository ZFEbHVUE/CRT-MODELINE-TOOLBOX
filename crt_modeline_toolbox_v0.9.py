...
# Format: hmin, hmax, vfmin, vfmax, hfp, hs, hbp, vfp, vs, vbp, pLmin, pLmax, iLmin, iLmax
# ==============================================================================
PRESETS = {
    "PAL TV": {
...
def calculate_from_range(H, V, Hfreq, Vfreq, interlaced, r):
    ...
    TERM_H = H / (H_T * (1.0 - hfp_t/H_T - hs_t/H_T - hbp_t/H_T))
    HFP   = round(hfp_t * TERM_H)
    HSYNC = round(hs_t  * TERM_H)
    HBP   = round(hbp_t * TERM_H)

    TERM_V = V / (V_T * (1.0 - vfp_t/V_T - vs_t/V_T - vbp_t/V_T))
    VFP   = round(vfp_t * TERM_V)
    VSYNC = round(vs_t  * TERM_V)
    VBP   = round(vbp_t * TERM_V)
...
def verify_modeline(modeline_str):
    parts = modeline_str.strip().split()
    if len(parts) < 9:
        return None
    try:
        pclk = float(parts[0])
        ...
        return None
    interlaced = "interlace" in modeline_str.lower()
    Div = 2 if interlaced else 1
    Hfreq = pclk * 1e6 / X4
    Vfreq = Hfreq * Div / Y4
    HFP,HSYNC,HBP = X2-X1, X3-X2, X4-X3
    VFP,VSYNC,VBP = Y2-Y1, Y3-Y2, Y4-Y3
    return dict(X1=X1,X2=X2,X3=X3,X4=X4,Y1=Y1,Y2=Y2,Y3=Y3,Y4=Y4,
                HFP=HFP,HSYNC=HSYNC,HBP=HBP,VFP=VFP,VSYNC=VSYNC,VBP=VBP,
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("CRT Modeline Toolbox — ZFEbHVUE v1.0")
    ...
    def _style(self):
        ...
        s.configure("TCombobox", fieldbackground=BG2, foreground=FG, selectbackground=BG3)
    ...
    def _text_ro(self, parent, height=2, width=80):
        t = tk.Text(parent, height=height, width=width, bg=BG2, fg=YELLOW,
                    font=FONT_M, relief="flat", bd=4, state="disabled",
    ...
    def _build_gen(self, parent):
        ...
        frm_p.grid(row=0, column=0, padx=6, pady=4, sticky="nsew")
        frm_p.columnconfigure(1, weight=1)

        ttk.Label(frm_p, text="Monitor", foreground=FG2, font=FONT_S).grid(
            row=0, column=0, padx=4, pady=2, sticky="w")
        self.cmb_preset = ttk.Combobox(frm_p, values=list(PRESETS.keys()),
                                        state="readonly", width=28)
        self.cmb_preset.set("Arcade 15kHz")
        self.cmb_preset.grid(row=0, column=1, columnspan=3, padx=4, pady=2, sticky="ew")
        self.cmb_preset.bind("<<ComboboxSelected>>", lambda e: self._apply_preset())

        ttk.Label(frm_p, text="Range", foreground=FG2, font=FONT_S).grid(
            row=1, column=0, padx=4, pady=2, sticky="w")
        self.cmb_range = ttk.Combobox(frm_p, values=[], state="readonly", width=16)
        self.cmb_range.grid(row=1, column=1, padx=4, pady=2, sticky="w")
        self.cmb_range.bind("<<ComboboxSelected>>", lambda e: self._on_range_select())
        self.lbl_range_info = ttk.Label(frm_p, text="", foreground=YELLOW, font=FONT_S)
        self.lbl_range_info.grid(row=1, column=2, columnspan=2, padx=4, sticky="w")

        for lbl, attr, row, col, default in [
            ("H Width",     "e_width",  2, 0, "640"),
            ("V Height",    "e_height", 2, 2, "480"),
            ("Hfreq (kHz)", "e_hfreq",  3, 0, "15.690"),
            ("Vfreq (Hz)",  "e_vfreq",  3, 2, "60.0"),
        ]:
            ttk.Label(frm_p, text=lbl, foreground=FG2, font=FONT_S).grid(
                row=row, column=col, padx=4, pady=2, sticky="w")
            e = ttk.Entry(frm_p, width=10)
            e.insert(0, default)
            e.grid(row=row, column=col+1, padx=4, pady=2, sticky="w")
            setattr(self, attr, e)

        self.e_width.bind("<FocusOut>",  lambda e: self._calc_gen())
        self.e_width.bind("<Return>",    lambda e: self._calc_gen())
        self.e_height.bind("<FocusOut>", lambda e: self._calc_gen())
        self.e_height.bind("<Return>",   lambda e: self._calc_gen())
        self.e_hfreq.bind("<FocusOut>",  lambda e: self._on_hfreq_change())
        self.e_hfreq.bind("<Return>",    lambda e: self._on_hfreq_change())
        self.e_vfreq.bind("<FocusOut>",  lambda e: self._calc_gen())
        self.e_vfreq.bind("<Return>",    lambda e: self._calc_gen())

        ttk.Label(frm_p, text="Interlaced", foreground=FG2, font=FONT_S).grid(
            row=4, column=0, padx=4, pady=2, sticky="w")
        self.var_interlaced = tk.IntVar(value=0)
        tk.Checkbutton(frm_p, variable=self.var_interlaced,
                       bg=BG, fg=FG, selectcolor=BG3, activebackground=BG,
                       command=self._calc_gen).grid(row=4, column=1, sticky="w", padx=4)

        ttk.Label(frm_p, text="Output", foreground=FG2, font=FONT_S).grid(
            row=4, column=2, padx=4, pady=2, sticky="w")
        self.e_output = ttk.Entry(frm_p, width=10)
        self.e_output.insert(0, "DP-1")
        self.e_output.grid(row=4, column=3, padx=4, pady=2, sticky="w")
        ...
        frm_r.grid(row=0, column=1, padx=6, pady=4, sticky="nsew")

        fields = [
            ("H Front Porch (µs)", "e_hfp", "2.000"),
            ("H Sync (µs)",        "e_hs",  "4.700"),
            ("H Back Porch (µs)",  "e_hbp", "8.000"),
            ("V Front Porch (ms)", "e_vfp", "0.064"),
            ("V Sync (ms)",        "e_vs",  "0.192"),
            ("V Back Porch (ms)",  "e_vbp", "1.024"),
        ]
        for i, (lbl, attr, default) in enumerate(fields):
            ttk.Label(frm_r, text=lbl, foreground=FG2, font=FONT_S).grid(
                row=i, column=0, padx=4, pady=2, sticky="w")
            e = ttk.Entry(frm_r, width=10)
            e.insert(0, default)
            e.grid(row=i, column=1, padx=4, pady=2)
            e.bind("<FocusOut>", lambda ev: self._calc_gen())
            e.bind("<Return>",   lambda ev: self._calc_gen())
            setattr(self, attr, e)

        ttk.Label(frm_r, text="Active range", foreground=FG2, font=FONT_S).grid(
            row=6, column=0, padx=4, pady=2, sticky="w")
        self.lbl_range_badge = ttk.Label(frm_r, text="15KHz", foreground=ACCENT, font=FONT_B)
        self.lbl_range_badge.grid(row=6, column=1, padx=4, pady=2, sticky="w")

        # CRT Range fixed
        frm_cf = ttk.LabelFrame(parent, text="CRT Range — Calamity values (fixed to preset)")
        frm_cf.grid(row=1, column=0, columnspan=2, padx=6, pady=4, sticky="ew")
        ...
        frm_cc.grid(row=2, column=0, columnspan=2, padx=6, pady=2, sticky="ew")
        ...
        frm_t.grid(row=3, column=0, padx=6, pady=4, sticky="nsew")
        cols = ("Parameter", "px/lines", "Time", "Unit")
        self.tree_gen = ttk.Treeview(frm_t, columns=cols, show="headings", height=10)
        for c in cols:
            w = 130 if c == "Parameter" else 70
            self.tree_gen.heading(c, text=c)
            self.tree_gen.column(c, width=w, anchor="center")
        self.tree_gen.pack(fill="both", expand=True, padx=4, pady=4)

        # Results panel
        frm_res = ttk.Frame(parent)
        frm_res.grid(row=3, column=1, padx=6, pady=4, sticky="nsew")
        ...
        self.lbl_chk_h = ttk.Label(frm_chk, text="", font=FONT_S)
        self.lbl_chk_h.pack(anchor="w", padx=6, pady=1)
        self.lbl_chk_v = ttk.Label(frm_chk, text="", font=FONT_S)
        self.lbl_chk_v.pack(anchor="w", padx=6, pady=1)
        self.lbl_chk_l = ttk.Label(frm_chk, text="", font=FONT_S)
        ...
        ttk.Button(frm_btn, text="Copy xrandr",  command=self._copy_xrandr).pack(side="left", padx=4)
        ttk.Button(frm_btn, text="Apply xrandr", command=self._apply_xrandr).pack(side="left", padx=4)
    ...
    def _build_ver(self, parent):
        ...
        ttk.Label(frm_in,
                  ...
                  foreground=FG2, font=FONT_S).pack(anchor="w", padx=4)
        self.e_modeline_in = ttk.Entry(frm_in, width=80, font=FONT_M)
        ...
        ttk.Label(frm_sel, text="Monitor:", foreground=FG2, font=FONT_S).pack(side="left", padx=4)
        ...
        ttk.Label(frm_sel, text="Output:", foreground=FG2, font=FONT_S).pack(side="left", padx=8)
        self.e_output_v = ttk.Entry(frm_sel, width=10)
        self.e_output_v.insert(0, "DP-1")
        self.e_output_v.pack(side="left", padx=4)
        ttk.Button(frm_sel, text="Verify", command=self._calc_ver).pack(side="left", padx=8)
        self.lbl_range_v = ttk.Label(frm_sel, text="", foreground=ACCENT, font=FONT_B)
        self.lbl_range_v.pack(side="left", padx=4)

        frm_t = ttk.LabelFrame(parent, text="Timings")
        frm_t.grid(row=2, column=0, padx=6, pady=4, sticky="nsew")
        cols = ("Parameter", "px/lines", "Time", "Unit")
        self.tree_ver = ttk.Treeview(frm_t, columns=cols, show="headings", height=10)
        for c in cols:
            w = 130 if c == "Parameter" else 70
            self.tree_ver.heading(c, text=c)
            self.tree_ver.column(c, width=w, anchor="center")
        ...
        frm_chk.pack(fill="x", pady=4)
        self.lbl_v_chk_h = ttk.Label(frm_chk, text="", font=FONT_S)
        self.lbl_v_chk_h.pack(anchor="w", padx=6, pady=1)
        self.lbl_v_chk_v = ttk.Label(frm_chk, text="", font=FONT_S)
        self.lbl_v_chk_v.pack(anchor="w", padx=6, pady=1)
        self.lbl_v_chk_l = ttk.Label(frm_chk, text="", font=FONT_S)
    ...
    def _metric(self, parent, label, row, col):
        ...
        ttk.Label(f, text=label, foreground=FG2, font=FONT_S, background=BG2).pack(anchor="w")
        lbl = ttk.Label(f, text="—", foreground=FG, font=("Sans",11,"bold"), background=BG2)
        lbl.pack(anchor="w")
        return lbl
    ...
    def _apply_preset(self):
        ...
        if not p:
            return
        self.e_hfreq.delete(0, "end"); self.e_hfreq.insert(0, str(p["dH"]))
        self.e_vfreq.delete(0, "end"); self.e_vfreq.insert(0, str(p["dV"]))
        labels = [x["lb"] for x in p["ranges"]]
        self.cmb_range["values"] = labels
        self.cmb_range.set(labels[0])
        if len(p["ranges"]) > 1:
            self.lbl_range_info.config(text="← select range")
        else:
            self.lbl_range_info.config(text="")
        r = p["ranges"][0]
        self._load_range(r)
        self._calc_gen()
    ...
    def _load_range(self, r):
        ...
        for e, k in [(self.e_hfp,"hfp"),(self.e_hs,"hs"),(self.e_hbp,"hbp"),
                     (self.e_vfp,"vfp"),(self.e_vs,"vs"),(self.e_vbp,"vbp")]:
            e.delete(0, "end"); e.insert(0, str(r[k]))
        self.lbl_range_badge.config(text=r["lb"])
        # Sync combobox to current range label
        if r["lb"] in self.cmb_range["values"]:
            self.cmb_range.set(r["lb"])
        # Update Hfreq to midpoint of selected range
        mid = (r["hmin"] + r["hmax"]) / 2 / 1000
        self.e_hfreq.delete(0, "end"); self.e_hfreq.insert(0, f"{mid:.3f}")
        # Fixed CRT Range — updated here only
    ...
    def _on_hfreq_change(self):
        name = self.cmb_preset.get()
        try:
            hfreq_hz = float(self.e_hfreq.get()) * 1000
        except:
            return
    ...
    def _calc_gen(self):
        try:
            H  = int(self.e_width.get())
            V  = int(self.e_height.get())
            Hf = float(self.e_hfreq.get()) * 1000
            Vf = float(self.e_vfreq.get())
            i  = self.var_interlaced.get()
            r  = self._current_range or PRESETS["Arcade 15kHz"]["ranges"][0]
        except:
            return

        res = calculate_from_range(H, V, Hf, Vf, i, r)
        t   = calc_timings(res)
        ...
        self.lbl_chk_h.config(
            text=f"{'✓' if hok else '✗'}  Hfreq {Hf/1000:.3f} kHz ∈ [{r['hmin']}–{r['hmax']}] Hz",
            foreground=GREEN if hok else RED)
        self.lbl_chk_v.config(
            text=f"{'✓' if vok else '✗'}  Vfreq {res['Vfreq_actual']:.3f} Hz ∈ [{r['vfmin']}–{r['vfmax']}] Hz",
            foreground=GREEN if vok else RED)
        self.lbl_chk_l.config(
            text=f"{'✓' if lok else '✗'}  V={V} lines ∈ {lstr}",
            foreground=GREEN if lok else RED)
