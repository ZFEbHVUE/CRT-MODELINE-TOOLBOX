...
def optimize_modeline(H, V, Hfreq_target, Vfreq_target, interlaced, r):
    ...
    # SwitchRes: max_vfreq = hmax / (V/2 + round_near(hmax × vblank_s))
    # If round_near(hmax × vblank_s) rounds UP (≥ X.5), vfreq_real drops below
    # target and Hfreq shifts by ~30 Hz. Fix: reduce VBP by 1 if needed.
    hmax = r["hmax"]
    threshold_ms = 22500.0 / hmax   # = 22.5/hmax × 1000
    Vf_act = Hfreq * Div / best_V_total
    line_ms = 1000.0 / (Hfreq * Div)  # duration of one V line in ms (interlaced: /2)
    vfp_ms  = VFP  * line_ms
    vs_ms   = VSYNC * line_ms
    vbp_ms  = VBP  * line_ms
    vblank_ms = vfp_ms + vs_ms + vbp_ms
...
class App(tk.Tk):
    def __init__(self):
        ...
        self._last_range    = None   # last live range for geometry re-render
        self._last_rg       = None   # last geometry-adjusted result for diagram
    ...
    def _build_gen(self, parent):
        ...
        # ---- Geometry (SwitchRes -g) ----
        frm_geo = ttk.LabelFrame(parent, text="Geometry  —  Move / Shift  (SwitchRes: -g  h_size:h_shift:v_shift)")
        frm_geo.grid(row=4, column=0, columnspan=2, padx=6, pady=4, sticky="ew")

        def _geo_spin(parent, label, from_, to, init, step=1, fmt="%.0f", width=6, is_float=False):
            frm = tk.Frame(parent, bg=BG)
            frm.pack(side="left", padx=12)
            tk.Label(frm, text=label, fg=FG2, bg=BG, font=FONT_S,
                     anchor="w").pack(anchor="w")
            spn = tk.Spinbox(frm, from_=from_, to=to, increment=step,
                              format=fmt, width=width,
                              bg=BG2, fg=YELLOW, insertbackground=FG,
                              buttonbackground=BG2, relief="flat", font=FONT_M,
                              command=self._apply_geometry)
            spn.delete(0, "end"); spn.insert(0, (f"{init:.2f}" if is_float else str(init)))
            spn.bind("<Return>",   lambda e: self._apply_geometry())
            spn.bind("<FocusOut>", lambda e: self._apply_geometry())
            spn.pack()
            return spn

        frm_geo_row = tk.Frame(frm_geo, bg=BG)
        frm_geo_row.pack(fill="x", padx=4, pady=6)

        self.spn_hmove  = _geo_spin(frm_geo_row, "H move (px)\n= H_shift", -200, 200, 0)
        self.spn_vmove  = _geo_spin(frm_geo_row, "V move (lines)\n= V_shift", -100, 100, 0)
        self.spn_hsize  = _geo_spin(frm_geo_row, "H_size\n(zoom H)", 0.50, 2.00, 1.00,
                                     step=0.01, fmt="%.2f", is_float=True)
        self.spn_vsize  = _geo_spin(frm_geo_row, "V_size ⚗\n(zoom V, relance opt.)", 0.50, 2.00, 1.00,
                                     step=0.01, fmt="%.2f", width=6, is_float=True)

        self.lbl_geo_info = tk.Label(frm_geo, text="", fg=YELLOW, bg=BG, font=FONT_S)
        self.lbl_geo_info.pack(anchor="w", padx=16, pady=(0,4))

        # ---- CRT Range displays side by side (row 5) ----
        frm_ranges = ttk.Frame(parent)
        frm_ranges.grid(row=5, column=0, columnspan=2, padx=6, pady=2, sticky="ew")
        ...
        frm_t.grid(row=6, column=0, padx=6, pady=4, sticky="nsew")
        ...
        frm_res.grid(row=6, column=1, padx=6, pady=4, sticky="nsew")
        ...
        self.txt_xrandr = self._text_ro(frm_xr, height=4, width=50)
        self.txt_xrandr.pack(fill="x", padx=4, pady=4)

        frm_btn = ttk.Frame(frm_res)
        frm_btn.pack(fill="x", pady=(4,0))

        # Row 1 — xrandr + optimiser + Lock VFP
        frm_btn1 = ttk.Frame(frm_btn)
        frm_btn1.pack(fill="x", pady=(0,2))
        ttk.Button(frm_btn1, text="Copy xrandr",
                   command=self._copy_xrandr).pack(side="left", padx=4)
        ttk.Button(frm_btn1, text="Apply xrandr",
                   command=self._apply_xrandr).pack(side="left", padx=4)
        ttk.Button(frm_btn1, text="⚡ Optimise (LS)",
                   command=self._optimise_ls).pack(side="left", padx=(16, 4))
        self.var_vfp_lock = tk.IntVar(value=0)
        ttk.Checkbutton(frm_btn1, text="Δ VFP", variable=self.var_vfp_lock,
                        style="TCheckbutton").pack(side="left", padx=(0, 2))
        self.spn_vfp_lock = tk.Spinbox(frm_btn1, from_=-50, to=50, width=4,
                                        bg=BG2, fg=YELLOW, insertbackground=FG,
                                        buttonbackground=BG2, relief="flat",
                                        font=FONT_S)
        self.spn_vfp_lock.delete(0, "end"); self.spn_vfp_lock.insert(0, "3")
        self.spn_vfp_lock.pack(side="left", padx=(0, 4))
        tk.Label(frm_btn1, text="lines", fg=FG2, bg=BG, font=FONT_S).pack(side="left", padx=(0,8))
    ...
    def _optimise_ls(self):
        ...
        # Apply VFP lock if enabled — force VFP to N lines, redistribute to VBP
        if self.var_vfp_lock.get():
            try:
                vfp_val = int(self.spn_vfp_lock.get())
            except ValueError:
                vfp_val = None
            if vfp_val is not None and vfp_val != 0:
                # >0 : valeur absolue   <0 : offset relatif
                if vfp_val > 0:
                    new_vfp = vfp_val
                else:
                    new_vfp = max(1, res["VFP"] + vfp_val)
        ...
        # Show Vfreq error + SwitchRes threshold status
        ve = res["Vfreq_error"]
        if ve < 1e-6:
            ve_txt = "Vfreq error: 0.000000 Hz ✓"
            ve_fg  = GREEN
        else:
            ve_txt = f"Vfreq error: {ve:.6f} Hz"
            ve_fg  = YELLOW

        sr_ok  = res.get("switchres_ok", True)
        vb_ms  = res.get("vblank_ms", 0)
        thr_ms = res.get("threshold_ms", 0)
        if sr_ok:
            sr_txt = f"  |  SR V-blank: {vb_ms:.3f}/{thr_ms:.3f} ms ✓"
            sr_fg  = GREEN
        else:
            sr_txt = f"  |  SR V-blank: {vb_ms:.3f}/{thr_ms:.3f} ms ✗ VBP réduit"
            sr_fg  = YELLOW

        self.lbl_ls_error.config(text=ve_txt + sr_txt, fg=ve_fg if sr_ok else sr_fg)
    ...
    def _apply_geometry(self, *_):
        """Re-render modeline/xrandr with current geometry offsets.
        All geometry (H_size, V_size, H_move, V_move) is applied
        in _refresh_display from the base _last_res."""
        if not self._last_res or not self._last_range:
            return
        self._refresh_display(self._last_res, self._last_range)
    ...
    def _refresh_display(self, res, r_live):
        self._last_res   = res
        self._last_range = r_live

        # ── Apply geometry offset to produce res_geo ──────────────────────────
        try:   hmove = int(self.spn_hmove.get())
        except: hmove = 0
        try:   vmove = int(self.spn_vmove.get())
        except: vmove = 0
        try:   hsize = float(self.spn_hsize.get())
        except: hsize = 1.0

        rg = dict(res)  # shallow copy

        # H_size: scale H blanking → changes H_total and pclk, keeps H_active & Hfreq
        if abs(hsize - 1.0) > 0.001:
            H_blank_orig = res["X4"] - res["X1"]
            H_blank_new  = max(3, round(H_blank_orig / hsize))
            HFP_new  = max(1, round(res["HFP"]   / hsize))
            HS_new   = max(1, round(res["HSYNC"]  / hsize))
            HBP_new  = H_blank_new - HFP_new - HS_new
            if HBP_new < 1: HBP_new = 1
            X4_new = res["X1"] + HFP_new + HS_new + HBP_new
            rg["HFP"]   = HFP_new;  rg["HSYNC"]  = HS_new;   rg["HBP"]   = HBP_new
            rg["X2"]    = res["X1"] + HFP_new
            rg["X3"]    = rg["X2"]  + HS_new
            rg["X4"]    = X4_new
            rg["pclk"]  = res["Hfreq"] * X4_new / 1e6

        # V_size: scale total V blanking (symmetric with H_size)
        try:   vsize = float(self.spn_vsize.get())
        except: vsize = 1.0
        if abs(vsize - 1.0) > 0.001:
            V_blank_orig = rg["VFP"] + rg["VSYNC"] + rg["VBP"]
            V_blank_new  = max(rg["VSYNC"] + 2, round(V_blank_orig / vsize))
            VS_new       = rg["VSYNC"]
            remaining    = V_blank_new - VS_new
            vfp_ratio    = rg["VFP"] / (rg["VFP"] + rg["VBP"]) if (rg["VFP"] + rg["VBP"]) > 0 else 0.2
            VFP_new      = max(1, round(remaining * vfp_ratio))
            VBP_new      = max(1, remaining - VFP_new)
            rg["VFP"]   = VFP_new;  rg["VSYNC"] = VS_new;  rg["VBP"]  = VBP_new
            rg["Y2"]    = rg["Y1"] + VFP_new
            rg["Y3"]    = rg["Y2"] + VS_new
            rg["Y4"]    = rg["Y3"] + VBP_new
            Div = 2 if rg["interlaced"] else 1
            rg["Vfreq_actual"] = rg["Hfreq"] * Div / rg["Y4"]

        # H_move: shift X2/X3, H_total unchanged — clamp to keep HFP >= 1
        hfp_cur = rg["X2"] - rg["X1"]
        hbp_cur = rg["X4"] - rg["X3"]
        hmove_max =  hfp_cur - 1   # shift right max
        hmove_min = -(hbp_cur - 1) # shift left max
        hmove_clamped = max(hmove_min, min(hmove_max, hmove))
        rg["X2"]  -= hmove_clamped;  rg["X3"]  -= hmove_clamped

        # V_move: shift Y2/Y3
        # If VFP or VBP would go < 1 → re-optimise with more blanking
        vfp_after = rg["Y2"] - rg["Y1"] - vmove
        vbp_after = rg["Y4"] - rg["Y3"] + vmove
        geo_warn = ""
        vmove_clamped = vmove
        if vfp_after < 1 or vbp_after < 1:
            extra_vfp = max(0, 1 - vfp_after)
            extra_vbp = max(0, 1 - vbp_after)
            line_ms   = 1000.0 / (rg["Hfreq"] * (2 if rg["interlaced"] else 1))
            r_adj = dict(r_live)
            r_adj["vfp"] = max(r_live["vfp"], (rg["VFP"] + extra_vfp) * line_ms)
            r_adj["vbp"] = max(r_live["vbp"], (rg["VBP"] + extra_vbp) * line_ms)
            new_res = optimize_modeline(
                rg["X1"], rg["Y1"], rg["Hfreq"], rg["Vfreq_actual"],
                rg["interlaced"], r_adj)
            rg = new_res
            rg["pclk"] = rg["Hfreq"] * rg["X4"] / 1e6
            # Apply move on the new modeline
            rg["Y2"] -= vmove;  rg["Y3"] -= vmove
            geo_warn = "  ⚠ re-optimisé"
        else:
            rg["Y2"] -= vmove;  rg["Y3"] -= vmove
        rg["HFP"]  = rg["X2"] - rg["X1"]
        rg["HBP"]  = rg["X4"] - rg["X3"]
        rg["VFP"]  = rg["Y2"] - rg["Y1"]
        rg["VBP"]  = rg["Y4"] - rg["Y3"]
        if hmove_clamped != hmove:
            geo_warn += f"  ⚠ H clampé à {hmove_clamped}"
        # ──────────────────────────────────────────────────────────────────────

        t   = calc_timings(rg)
        self._last_rg = rg   # for diagram

        # Sync px/lines sliders with geometry-adjusted values
        self.sw_hfp_px.set_value(rg["HFP"])
        self.sw_hs_px.set_value( rg["HSYNC"])
        self.sw_hbp_px.set_value(rg["HBP"])
        self.sw_vfp_px.set_value(rg["VFP"])
        self.sw_vs_px.set_value( rg["VSYNC"])
        self.sw_vbp_px.set_value(rg["VBP"])
        Hf  = rg["Hfreq"]
        V   = rg["Y1"]
        i   = rg["interlaced"]
        out = self.e_output.get() or "DP-1"

        self.lbl_pclk.config( text=f"{rg['pclk']:.6f} MHz")
        self.lbl_hfreq.config(text=f"{Hf/1000:.6f}")
        self.lbl_vfreq.config(text=f"{rg['Vfreq_actual']:.6f}")
        self.lbl_htot.config( text=str(rg["X4"]))
        self.lbl_vtot.config( text=str(rg["Y4"]))
        self.lbl_hblk.config( text=str(rg["HFP"]+rg["HSYNC"]+rg["HBP"]))

        self.tree_gen.delete(*self.tree_gen.get_children())
        for row in [
            ("H active",      rg["X1"],    f"{t['H_actif_us']:.3f}", "µs"),
            ("H Front Porch", rg["HFP"],   f"{t['HFP_us']:.3f}",     "µs"),
            ("H Sync",        rg["HSYNC"], f"{t['HSYNC_us']:.3f}",   "µs"),
            ("H Back Porch",  rg["HBP"],   f"{t['HBP_us']:.3f}",     "µs"),
            ("H total",       rg["X4"],    f"{t['H_total_us']:.3f}", "µs"),
            ("—", "", "", ""),
            ("V active",      rg["Y1"],    f"{t['V_actif_ms']:.3f}", "ms"),
            ("V Front Porch", rg["VFP"],   f"{t['VFP_ms']:.3f}",     "ms"),
            ("V Sync",        rg["VSYNC"], f"{t['VSYNC_ms']:.3f}",   "ms"),
            ("V Back Porch",  rg["VBP"],   f"{t['VBP_ms']:.3f}",     "ms"),
            ("V total",       rg["Y4"],    f"{t['V_total_ms']:.3f}", "ms"),
        ]:
            self.tree_gen.insert("", "end", values=row)

        hok = r_live["hmin"] <= round(Hf) <= r_live["hmax"]
        vok = r_live["vfmin"] <= round(rg["Vfreq_actual"], 2) <= r_live["vfmax"]
        if i:
        ...
        sr_geo = f"{hsize:.2f}:{hmove_clamped}:{vmove_clamped}"
        if hasattr(self, "lbl_geo_info"):
            self.lbl_geo_info.config(
                text=f"HFP={rg['HFP']}  HBP={rg['HBP']}  VFP={rg['VFP']}  VBP={rg['VBP']}   -g \"{sr_geo}\"{geo_warn}",
                fg=YELLOW if geo_warn else FG2)
        ml = (f"{rg['pclk']:.6f} {rg['X1']} {rg['X2']} {rg['X3']} {rg['X4']} "
        ...
        self._set_text(self.txt_xrandr,
                       ...
                       f'switchres {rg["X1"]} {rg["Y1"]} {rg["Vfreq_actual"]:.2f}'
                       f' -g "{sr_geo}" -i /etc/switchres.ini')
    ...
    def _show_diagram(self):
        ...
        res = self._last_rg or self._last_res
        if not res:
