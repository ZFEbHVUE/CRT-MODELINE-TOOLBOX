...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("CRT Modeline Toolbox — CRT v2.5")
    ...
    def _build_result(self, parent):
        ...
        # Geometry
        frm_geo = ttk.LabelFrame(inner, text="Geometry  -g  h_size:h_shift:v_shift")
        frm_geo.pack(fill="x", padx=4, pady=(0,2))
        frm_geo_row = tk.Frame(frm_geo, bg=BG)
        frm_geo_row.pack(fill="x", padx=4, pady=4)

        def _geo_spin(parent, label, from_, to, init, step=1, fmt="%.0f", width=5, is_float=False):
            f = tk.Frame(parent, bg=BG); f.pack(side="left", padx=8)
            tk.Label(f, text=label, fg=FG2, bg=BG, font=FONT_S).pack(anchor="w")
            spn = tk.Spinbox(f, from_=from_, to=to, increment=step, format=fmt, width=width,
        ...
        ttk.Checkbutton(frm_btn1, text="Δ VFP", variable=self.var_vfp_lock).pack(
            side="left", padx=(6,2))
    ...
    def _apply_geometry(self, *_):
        if self._last_res and self._last_range:
    ...
    def _refresh_display(self, res, r_live):
        ...
        # H_size: scale H blanking
        if abs(hsize - 1.0) > 0.001:
            Hb = max(3, round((res["X4"]-res["X1"]) / hsize))
            Hfp = max(1, round(res["HFP"] / hsize))
            Hs  = max(1, round(res["HSYNC"] / hsize))
            Hbp = max(1, Hb - Hfp - Hs)
            rg["HFP"]=Hfp; rg["HSYNC"]=Hs; rg["HBP"]=Hbp
            rg["X2"]=res["X1"]+Hfp; rg["X3"]=rg["X2"]+Hs
            rg["X4"]=rg["X3"]+Hbp
            rg["pclk"]=res["Hfreq"]*rg["X4"]/1e6

        # V_size: scale total V blanking
        if abs(vsize - 1.0) > 0.001:
            Vb = rg["VFP"]+rg["VSYNC"]+rg["VBP"]
            Vb_new = max(rg["VSYNC"]+2, round(Vb / vsize))
            rem = Vb_new - rg["VSYNC"]
            r_vfp = rg["VFP"]/(rg["VFP"]+rg["VBP"]) if (rg["VFP"]+rg["VBP"])>0 else 0.2
            Vfp = max(1, round(rem * r_vfp))
            Vbp = max(1, rem - Vfp)
            rg["VFP"]=Vfp; rg["VBP"]=Vbp
            rg["Y2"]=rg["Y1"]+Vfp; rg["Y3"]=rg["Y2"]+rg["VSYNC"]
            rg["Y4"]=rg["Y3"]+Vbp
            Div=2 if rg["interlaced"] else 1
            rg["Vfreq_actual"]=rg["Hfreq"]*Div/rg["Y4"]

        # H_move: clamp to keep HFP/HBP >= 1
        hmax_mv = rg["X2"]-rg["X1"]-1; hmin_mv = -(rg["X4"]-rg["X3"]-1)
        hmove_c = max(hmin_mv, min(hmax_mv, hmove))
        rg["X2"] -= hmove_c; rg["X3"] -= hmove_c

        # V_move: re-optimise if porch would go < 1
        vfp_after = rg["Y2"]-rg["Y1"]-vmove
        vbp_after = rg["Y4"]-rg["Y3"]+vmove
        geo_warn = ""
        if vfp_after < 1 or vbp_after < 1:
            ex_vfp = max(0, 1-vfp_after); ex_vbp = max(0, 1-vbp_after)
            lms    = 1000.0/(rg["Hfreq"]*(2 if rg["interlaced"] else 1))
            r_adj  = dict(r_live)
            r_adj["vfp"] = max(r_live["vfp"], (rg["VFP"]+ex_vfp)*lms)
            r_adj["vbp"] = max(r_live["vbp"], (rg["VBP"]+ex_vbp)*lms)
            nr = optimize_modeline(rg["X1"],rg["Y1"],rg["Hfreq"],rg["Vfreq_actual"],
                                   rg["interlaced"], r_adj)
        ...
        if hmove_c != hmove: geo_warn += f"  ⚠ H→{hmove_c}"

        self._last_rg = rg
        # ─────────────────────────────────────────────────────────────────────

        t=calc_timings(rg)
        Hf=rg["Hfreq"]; V=rg["Y1"]; i=rg["interlaced"]
        out=self.e_output.get() or "DP-1"

        self.lbl_pclk.config( text=f"{rg['pclk']:.4f} MHz")
        ...
        lok=(r_live["iLmin"]<=V<=r_live["iLmax"]) if i else (r_live["pLmin"]<=V<=r_live["pLmax"])
        lstr=(f"[{r_live['iLmin']}–{r_live['iLmax']}] int") if i else (f"[{r_live['pLmin']}–{r_live['pLmax']}] prog")
        self.lbl_chk_h.config(text=f"{'✓' if hok else '✗'}  {Hf/1000:.3f} kHz ∈ [{r_live['hmin']}–{r_live['hmax']}]",fg=GREEN if hok else RED)
        self.lbl_chk_v.config(text=f"{'✓' if vok else '✗'}  {rg['Vfreq_actual']:.3f} Hz ∈ [{r_live['vfmin']}–{r_live['vfmax']}]",fg=GREEN if vok else RED)
        self.lbl_chk_l.config(text=f"{'✓' if lok else '✗'}  V={V} ∈ {lstr}",fg=GREEN if lok else RED)
        ...
        H=rg["X1"]; auto_name=f"{H}x{V}{'i' if i else ''}"
        name=self.e_name.get().strip() or auto_name
        sr_geo=f"{hsize:.2f}:{hmove_c}:{vmove}"
    ...
    def _optimise_ls(self):
        ...
        # Δ VFP: >0 absolu, <0 offset relatif, 0 = pas de modification
        if self.var_vfp_lock.get():
            try: vfp_val=int(self.spn_vfp_lock.get())
            except ValueError: vfp_val=0
            if vfp_val != 0:
